...
import de.xthelegend.projectredextended.lib.istutils;
/**
 *
 * @author MineMaarten
 */
        ...
        for (IRecipe recipe : recipes) {
                    ...
                    if (recipe instanceof ShapedRecipes) {
                        ...
                        if (shaped.recipeItems != null) {
                            for (ItemStack input : shaped.recipeItems) {
                                if (input != null && istutils.isItemFuzzyEqual(input, recyclingItem)) {
                                    ItemStack moltenDownItem = getRecyclingStack(recyclingItem);
                                    if (currentlyRecycledInto == null
                                            || istutils.isItemFuzzyEqual(currentlyRecycledInto, moltenDownItem)) {
                            ...
                            for (Object input : shapedOreRecipe.getInput()) {
                                if (input != null) {
                                    ...
                                    for (ItemStack item : itemList) {
                                        if (item != null && istutils.isItemFuzzyEqual(item, recyclingItem)) {
                                            ItemStack moltenDownItem = getRecyclingStack(recyclingItem);
                                            if (currentlyRecycledInto == null
                                                    || istutils.isItemFuzzyEqual(currentlyRecycledInto, moltenDownItem)) {
                                                currentlyRecycledInto = moltenDownItem;
            ...
            if (recyclingAmount > 0 && recipe.getRecipeOutput().stackSize > 0) {
                ...
                for (int i = 0; i < registeredRecycledItems.size(); i++) {
                    if (istutils.isItemFuzzyEqual(registeredRecycledItems.get(i), recipe.getRecipeOutput())) {
        ...
        private StandardAlloyFurnaceRecipe(ItemStack craftingResult, ItemStack... requiredItems) {
            ...
            for (ItemStack stack : requiredItems) {
                for (ItemStack stack2 : requiredItems) {
                    if (stack != stack2 && istutils.isItemFuzzyEqual(stack, stack2))
        ...
        public boolean matches(ItemStack[] input) {
            ...
            for (ItemStack requiredItem : requiredItems) {
                ...
                for (ItemStack inputStack : input) {
                    if (inputStack != null && istutils.isItemFuzzyEqual(inputStack, requiredItem)) {
